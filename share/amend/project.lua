...
if fs.exists(CMakeLists) then
    require "amend.use.cmake"
    ...
    if project then
        ...
        message(DEBUG, "CMake project:")
        for i,k in ipairs(project) do
            message(DEBUG, "    %q", k)
        end
        ...
        while #project > 0 do
            ...
            elseif cmd == "LANGUAGES" then
                break
            end
        end

        for _, U in ipairs(project) do
            local u = U:lower()
            local res, _ = pcall(require, "amend.use."..u)
            if res then
                PROJECT.USES = PROJECT.USES or {}
                table.insert(PROJECT.USES, U)
            end
        end
    end
end

io.dump(PROJECT, {key = "PROJECT"})
...
-- if PATHS[0] then
--     package:addpath(PATHS[0])
-- end
-- for _, path in ipairs(PATHS) do
--     package:addpath(ROOTDIR .. dirsep .. path)
-- end
